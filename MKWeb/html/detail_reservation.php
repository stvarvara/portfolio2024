...
if (isset($_GET["id_devis"])) {
    // Requête pour récupérer les données du devis
    $sql_devis = 'SELECT * FROM sae._devis WHERE id = ' . $_GET["id_devis"];
    $devis = request($sql_devis, true);

    $prix_total = floatval($devis['prix_ttc']) + floatval($devis['taxe_commission']) + floatval($devis['taxe_sejour']);

    $reservation = array(
        'id_logement' => $devis['id_logement'],
        'id_client' => $id_client,
        'date_reservation' => date('Y-m-d'),
        'date_debut' => (new DateTime($devis['date_debut']))->format('Y-m-d'),
        'date_fin' => (new DateTime($devis['date_fin']))->format('Y-m-d'),
        'nb_occupant' => $devis['nb_occupant'],
        'taxe_sejour' => $devis['taxe_sejour'],
        'taxe_commission' => $devis['taxe_commission'],
    ...
    $sql .= ' WHERE id = ' . $devis['id_logement'];
    $res = request($sql, 1);
    $base_tarif = $res['base_tarif'];

    // Insérer la réservation
    $id_resa = insert('sae._reservation', array_keys($reservation), array_values($reservation), true);
    ...
    $sql = "SELECT sae._reservation.*, sae._utilisateur.photo_profile FROM sae._reservation 
        ...
        WHERE sae._reservation.id=$id_resa AND id_client = $id_client";
...
// On recupère les données à afficher
    ...
    $info_client = request("SELECT u.nom, u.prenom, a.commune, a.code_postal, a.numero, a.nom_voie, a.pays
          FROM sae._compte_client c
          INNER JOIN sae._utilisateur u ON c.id = u.id
          INNER JOIN sae._adresse a ON u.id_adresse = a.id
          WHERE c.id = '$id_client'", true);
